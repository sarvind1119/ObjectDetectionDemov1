...
def object_detection(cap, window_name):
    while True:
        try:
            ...
            if not success:
                print(f"Error: Unable to fetch the frame from {window_name}. Skipping this camera.")
                break  # Exit the loop for this camera but keep the program running
            ...
            print(f"An unexpected error occurred for {window_name}: {e}. Skipping this camera.")
            break
...
def handle_all_cameras():
    ...
    for camera_id, rtsp_url in camera_urls.items():
        ...
        if cap.isOpened():
            ...
            print(f"Error: Unable to access feed from Camera {camera_id}. Skipping this camera.")
